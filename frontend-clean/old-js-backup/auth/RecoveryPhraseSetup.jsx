...
import { Shield, Copy, Check, AlertTriangle, Download, Eye, EyeOff, Printer } from 'lucide-react';
...
`;
  ...
  const handlePrint = () => {
    ...
    const wordsHtml = words.map((word, index) =>
      `<div class="word"><span class="num">${index + 1}.</span> ${word}</div>`
    ).join('');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/50 backdrop-blur-sm overflow-y-auto">
      <div className={`w-full max-w-2xl max-h-[95vh] flex flex-col my-auto rounded-2xl shadow-2xl border ${
        darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
      }`}>
        {/* Header - Fixed */}
        <div className={`p-3 sm:p-5 border-b flex-shrink-0 ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {/* Warning Banner */}
          <div className={`mx-3 sm:mx-5 mt-3 sm:mt-4 p-3 rounded-xl border-2 ${
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className={`font-semibold text-sm ${darkMode ? 'text-yellow-300' : 'text-yellow-800'}`}>
            ...
            <div className={`p-3 sm:p-4 rounded-xl border-2 ${
              ...
              {showPhrase ? (
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                  {words.map((word, index) => (
                    ...
                    >
                      ...
                      <span className={`font-mono text-xs sm:text-sm font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                        {word}
                      </span>
                    </div>
                  ))}
                ...
                <div className="text-center py-8">
                  <Eye className={`mx-auto mb-2 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`} size={36} />
        ...
        {/* Footer - Fixed */}
        <div className={`p-3 sm:p-4 border-t flex gap-2 flex-shrink-0 ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          {onSkip && (
            <button
              onClick={onSkip}

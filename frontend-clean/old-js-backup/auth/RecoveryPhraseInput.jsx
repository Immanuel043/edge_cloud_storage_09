...
export default function RecoveryPhraseInput({
  ...
  const getSuggestions = useCallback((input) => {
    ...
    return bip39Words
      .filter(word => word.startsWith(lowercaseInput))
      .slice(0, 5);
  }, [bip39Words]);

  // Update suggestions when focused input changes
  useEffect(() => {
    if (focusedIndex >= 0 && value[focusedIndex]) {
      setSuggestions(getSuggestions(value[focusedIndex]));
  ...
  const handlePaste = (e, index) => {
    ...
    if (words.length > 1) {
      ...
      const nextEmptyIndex = newValue.findIndex((w, i) => i >= index && (!w || w.trim() === ''));
  ...
  const baseInputClass = `w-full px-2 py-1.5 text-sm rounded border transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500`;
  const getInputClass = (word) => {
  ...
  const suggestionClass = (isHighlighted) => {
    if (darkMode) {
    ...
    <div className="recovery-phrase-input">
            ...
            <div key={index} className="relative pt-4">
              ...
              <div className="relative">
                <input
                  ...
                  aria-describedby={hasValue ? (isValid ? `word-${index}-valid` : `word-${index}-invalid`) : undefined}
                  ...
                  <div className={dropdownClass} role="listbox" aria-label="Word suggestions">
                    {suggestions.map((suggestion, i) => (
                      <button
                        ...
                        className={suggestionClass(highlightedSuggestion === i)}
                        role="option"
      ...
      <div className={`mt-3 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
        <div className="flex items-center justify-between">
          <span>
            {value.filter(w => isValidWord(w)).length} of 24 words entered
          </span>
      ...
      <p className={`mt-2 text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
        Tip: Press Tab or Space to move to next word. Paste your full phrase to auto-fill all fields.
      </p>
    </div>
  );
}

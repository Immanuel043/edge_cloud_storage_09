...
export function SubscriptionProvider({ children }) {
   ...
   */
  const upgrade = useCallback(async (planCode) => {
    try {
      ...
      showNotification('error', `Upgrade failed: ${err.message}`);
      throw err;
    }
   ...
   */
  const downgrade = useCallback(async (planCode) => {
    try {
      const plan = availablePlans.find(p => p.plan_code === planCode);
      ...
      showNotification('error', `Downgrade failed: ${err.message}`);
      throw err;
    }
   ...
   */
  const previewChange = useCallback(async (planCode) => {
    try {
   ...
   */
  const checkQuota = useCallback((fileSize) => {
   ...
   * Get plan by code
   */
  const getPlanByCode = useCallback((planCode) => {
   ...
   */
  const isUpgrade = useCallback((planCode) => {
   ...
   */
  const isDowngrade = useCallback((planCode) => {
    const plan = getPlanByCode(planCode);
    if (!plan || !subscription) return false;

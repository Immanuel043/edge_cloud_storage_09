...
export default function QuotaAlertsView({ darkMode, storageStats }) {
  ...
  const fetchData = async (isRefresh = false) => {
    try {
      ...
      if (predictionRes.status === 'fulfilled' && predictionRes.value.ok) {
        const predData = await predictionRes.value.json();
  ...
  const usagePercent = storageStats?.percentage_used || prediction?.usage_percent * 100 || 0;
        ...
        <div className={`rounded-lg p-6 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
          ...
          {prediction.model_type && (
            <p className={`mt-4 text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
              Prediction model: {prediction.model_type} | Last updated: {new Date(prediction.prediction_date).toLocaleString()}
            </p>
          )}
        </div>
      )}

...
export default function RecoverySettings() {
  ...
  const [phraseDownloaded, setPhraseDownloaded] = useState(false);
  ...
  const handleSetupPhrase = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to setup recovery phrase.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRotatePhrase = async () => {
    setLoading(true);
    setError('');
    setSuccess('');
    ...
    try {
      ...
      console.error('Failed to generate new recovery phrase:', err);
      setError(err.message || 'Failed to generate new recovery phrase.');
      setLoading(false);
    }
  };
  ...
  const handleConfirmRotation = async () => {
    ...
    try {
      ...
      console.error('Failed to rotate recovery phrase:', err);
      setError(err.message || 'Failed to rotate recovery phrase.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCloseModal = () => {
    if (confirmStep && !loading) {
      ...
      if (!phraseCopied && !phraseDownloaded) {
        if (!confirm('Are you sure? You have not saved your new recovery phrase. If you close now, you will need to generate a new one.')) {
          return;
        }
      }
    }
      ...
      <div className="p-6 space-y-6">
        ...
        {success && (
          <div className={`p-4 rounded-xl border ${
            darkMode ? 'bg-green-900/20 border-green-600/40' : 'bg-green-50 border-green-300'
          }`}>
            <div className="flex items-center gap-3">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className={`text-xs mt-1 ${darkMode ? 'text-yellow-400' : 'text-yellow-700'}`}>
                  Your recovery phrase is the only way to recover your account if you forget your password.
                  Keep it stored securely and never share it with anyone.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>
          <p>
            <strong>Why rotate?</strong> If you believe your recovery phrase may have been compromised,
            rotating it will generate a new phrase and invalidate the old one.
          </p>
        </div>
      </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className={`text-xs mt-1 ${darkMode ? 'text-red-400' : 'text-red-700'}`}>
                          Your current recovery phrase will become invalid immediately.
                          Make sure you're ready to save a new one.
                        </p>
                      </div>
                    </div>
                  </div>

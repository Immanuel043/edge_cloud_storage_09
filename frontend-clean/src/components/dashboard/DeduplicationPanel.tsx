...
/**
 * DeduplicationPanel - Displays deduplication analytics and savings
 */
const DeduplicationPanel: React.FC<DeduplicationPanelProps> = ({
  ...
  const runGarbageCollection = async (): Promise<void> => {
    ...
    try {
      await storageService.runGarbageCollection(''); // Token parameter required but not used (uses cookie)
      if (onRefresh) {

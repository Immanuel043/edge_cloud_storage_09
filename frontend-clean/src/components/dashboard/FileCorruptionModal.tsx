...
/**
 * FileCorruptionModal Component
 *
 * Displays an error modal when a file cannot be decrypted.
 */
      ...
      >
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className={`text-sm font-medium ${darkMode ? 'text-red-300' : 'text-red-800'}`}>
                  The file <span className="font-semibold">"{fileName}"</span> could not be
                  decrypted.
                </p>
                <p className={`text-xs mt-2 ${darkMode ? 'text-red-400' : 'text-red-700'}`}>
                  {errorMessage ||
                    'The file may have been tampered with or corrupted during storage.'}
                </p>
              </div>
            </div>
          </div>
          ...
          <details className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>
            ...
            <div
              className={`mt-2 p-3 rounded-lg font-mono ${darkMode ? 'bg-gray-900' : 'bg-gray-100'}`}
            >
              {errorMessage}
            </div>
          </details>

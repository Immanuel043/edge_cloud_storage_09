 ...
 *
 * Displays share link information after creating a share.
 */
      ...
      <div className={`p-6 rounded-lg max-w-md w-full ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <div className={`p-3 rounded mb-4 ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
          ...
          <div className="flex items-center gap-2">
            <p
              className={`text-sm break-all flex-1 ${darkMode ? 'text-white' : 'text-gray-900'}`}
            >
              {shareUrl}
            </p>
            <button
          ...
          {infoItems.map(({ Icon, label, value }) => (
            <div key={label} className="flex justify-between items-center py-2">
              <span className="flex items-center gap-2">
                <Icon size={16} className={darkMode ? 'text-gray-300' : 'text-gray-600'} />

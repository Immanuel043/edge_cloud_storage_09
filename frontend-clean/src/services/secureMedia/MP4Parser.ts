...
interface StszResult {
  defaultSize: number;
  sizes: number[];
  sampleCount: number;
}

interface MvhdResult {
  timescale: number;
  duration: number;
}

interface SeekTableEntry {
  time: number;
  byteOffset: number;
  sampleIndex: number;
  chunkIndex: number;
 ...
 */
function findAtom(view: DataView, start: number, end: number, atomType: string): AtomLocation | null {
  let offset = start;
 ...
 */
function findAtomPath(view: DataView, start: number, end: number, path: string): AtomLocation | null {
 ...
 */
function parseStts(view: DataView, atomOffset: number, atomSize: number): SttsEntry[] {
 ...
 */
function parseStsc(view: DataView, atomOffset: number, atomSize: number): StscEntry[] {
 ...
 */
function parseStco(view: DataView, atomOffset: number, atomSize: number, is64Bit = false): number[] {
 ...
 */
function parseStsz(view: DataView, atomOffset: number, atomSize: number): StszResult {
 ...
 */
function parseStss(view: DataView, atomOffset: number, atomSize: number): number[] {
 ...
 */
function extractVideoCodecString(view: DataView, stsdOffset: number, stsdSize: number): string {
 ...
 */
function extractAudioCodecString(view: DataView, stsdOffset: number, stsdSize: number): string {
 ...
 */
function parseMvhd(view: DataView, atomOffset: number): MvhdResult {
...
function buildSeekTable(
  ...
  const seekTable: SeekTableEntry[] = [];
  ...
  for (let chunkIdx = 0; chunkIdx < stco.length; chunkIdx++) {
    ...
    const chunkOffsetValue = stco[chunkIdx];
    if (chunkOffsetValue === undefined) continue;

    let chunkOffset = chunkOffsetValue;

    // Determine samples per chunk
    while (stscIndex + 1 < stsc.length) {
      const nextEntry = stsc[stscIndex + 1];
      if (!nextEntry || nextEntry.firstChunk > chunkNumber) break;
      stscIndex++;
    }
    ...
    for (let s = 0; s < samplesInChunk; s++) {
      ...
      if (sttsRemaining === 0 && sttsIndex + 1 < stts.length) {
        sttsIndex++;
        const sttsEntry = stts[sttsIndex];
        if (sttsEntry) {
          sttsRemaining = sttsEntry.sampleCount;
          currentDelta = sttsEntry.sampleDelta;
        }
      }
    }
  }
 ...
 */
export function parse(headerData: ArrayBuffer, chunkSize = 64 * 1024 * 1024): MP4ParseResult {
  ...
  if (mvhd) {
    const { timescale, duration } = parseMvhd(view, mvhd.offset);
        ...
        if (stbl) {
          ...
          if (sttsAtom && stscAtom && (stcoAtom || co64Atom) && stszAtom) {
            ...
            const stco = co64Atom
              ...
              : (stcoAtom ? parseStco(view, stcoAtom.offset, stcoAtom.size, false) : []);
 ...
 */
export function timeToByteOffset(timeSeconds: number, seekTable: SeekTableEntry[]): SeekResult {
  ...
  while (left < right) {
    ...
    const midEntry = seekTable[mid];
    if (midEntry && midEntry.time <= timeSeconds) {
      left = mid;
    } else {
      right = mid - 1;
    }
 ...
 */
export function byteOffsetToChunkIndex(byteOffset: number, chunkSize = 64 * 1024 * 1024): number {

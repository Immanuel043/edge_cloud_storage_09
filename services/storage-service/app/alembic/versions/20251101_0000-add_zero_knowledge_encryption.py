...
def upgrade():
    ...
    op.add_column(
        "users", sa.Column("zk_enabled", sa.Boolean, nullable=False, server_default="false")
    )
    ...
    op.add_column(
        "users", sa.Column("kdf_algorithm", sa.String(20), nullable=False, server_default="pbkdf2")
    )
    op.add_column(
        "users", sa.Column("kdf_iterations", sa.Integer, nullable=False, server_default="600000")
    )
    ...
    op.add_column(
        "users",
        sa.Column("recovery_phrase_enabled", sa.Boolean, nullable=False, server_default="false"),
    )
    op.add_column("users", sa.Column("recovery_encrypted_master_key", sa.Text, nullable=True))
    op.add_column(
        "users", sa.Column("recovery_phrase_hash", sa.String(64), nullable=True)
    )  # SHA-256 of phrase (for verification)
    op.add_column(
        "users", sa.Column("recovery_phrase_created_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "users",
        sa.Column("recovery_phrase_verified", sa.Boolean, nullable=False, server_default="false"),
    )

    # Create indexes for ZK users
    op.create_index(
        "idx_users_zk_enabled",
        "users",
        ["zk_enabled"],
        postgresql_where=sa.text("zk_enabled = true"),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "encrypted_master_key", sa.Text, nullable=False
        ),  # Master key encrypted with hardware key
    ...
    op.create_unique_constraint(
        "uq_social_recovery_user_share", "social_recovery_contacts", ["user_id", "share_number"]
    )
    ...
    # ========== ADD CLIENT-ENCRYPTED FLAG TO OBJECTS ==========
    op.add_column(
        "objects", sa.Column("client_encrypted", sa.Boolean, nullable=False, server_default="false")
    )
    op.add_column("objects", sa.Column("encryption_metadata", postgresql.JSONB, nullable=True))

    # Create index for filtering ZK files
    op.create_index(
        "idx_objects_client_encrypted",
        "objects",
        ["client_encrypted"],
        postgresql_where=sa.text("client_encrypted = true"),
    )

...
def upgrade() -> None:
    # Add missing columns to user_interactions table
    op.add_column('user_interactions', sa.Column('interaction_count', sa.Integer(), server_default='0', nullable=False))
    op.add_column('user_interactions', sa.Column('last_interaction_at', sa.DateTime(timezone=True), nullable=True))
    op.add_column('user_interactions', sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False))

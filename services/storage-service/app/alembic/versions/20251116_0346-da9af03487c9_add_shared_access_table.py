...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column('shared_with_user_id', UUID(as_uuid=True), sa.ForeignKey('users.id'), nullable=True),
        sa.Column('file_id', UUID(as_uuid=True), sa.ForeignKey('objects.id', ondelete='CASCADE'), nullable=True),
        sa.Column('folder_id', UUID(as_uuid=True), sa.ForeignKey('folders.id', ondelete='CASCADE'), nullable=True),

...
def upgrade():
    ...
    # =========================================================================
    op.add_column(
        "subscription_plans", sa.Column("razorpay_plan_id_monthly", sa.String(255), nullable=True)
    )
    op.add_column(
        "subscription_plans",
    ...
    op.add_column(
        "subscription_plans", sa.Column("razorpay_plan_id_yearly", sa.String(255), nullable=True)
    )
    ...
    # =========================================================================
    op.add_column(
        "user_subscriptions", sa.Column("razorpay_subscription_id", sa.String(255), nullable=True)
    )
    op.add_column(
        "user_subscriptions", sa.Column("razorpay_order_id", sa.String(255), nullable=True)
    )
    op.add_column(
        "user_subscriptions", sa.Column("razorpay_payment_id", sa.String(255), nullable=True)
    )
    op.add_column(
        "user_subscriptions",
        sa.Column(
            "payment_gateway", sa.String(20), nullable=True, comment="'razorpay' or 'stripe'"
        ),
    )

    # Create index for payment_gateway
    op.create_index("idx_subscriptions_payment_gateway", "user_subscriptions", ["payment_gateway"])

    # Create index for Razorpay subscription ID
    op.create_index(
        "idx_subscriptions_razorpay", "user_subscriptions", ["razorpay_subscription_id"]
    )

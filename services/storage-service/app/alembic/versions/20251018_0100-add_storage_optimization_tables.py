...
revision = 'storage_optimization_001'
down_revision = 'ml_quota_prediction_001'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, default=uuid4),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('analysis_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('storage_analysis.id', ondelete='CASCADE'), nullable=False),

        # Suggestion details
        sa.Column('suggestion_type', sa.String(50), nullable=False),  # 'tier_migration', 'deduplication', 'compression', 'cleanup'
        ...
        # Actions
        sa.Column('action_type', sa.String(50)),  # 'move_to_cold', 'delete_duplicates', 'compress', 'delete_old'
        sa.Column('action_details', postgresql.JSON),  # Specific file IDs, parameters, etc.

        # Status
        sa.Column('status', sa.String(20), default='pending'),  # 'pending', 'accepted', 'rejected', 'completed'
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, default=uuid4),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('suggestion_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('optimization_suggestions.id', ondelete='SET NULL'), nullable=True),
        ...
        # Status
        sa.Column('status', sa.String(20), default='pending'),  # 'pending', 'in_progress', 'completed', 'failed'

...
def upgrade():
    ...
    op.create_index(
        "idx_files_created_at", "objects", [sa.text("created_at DESC")], if_not_exists=True
    )
    ...
    op.create_index("idx_files_size", "objects", ["file_size"], if_not_exists=True)

    # Optimize filtering by file type (file browser, type-specific views)
    # Used in: search_by_type, file_browser_filters
    op.create_index("idx_files_file_type", "objects", ["mime_type"], if_not_exists=True)
    ...
    op.create_index("idx_files_storage_tier", "objects", ["storage_tier"], if_not_exists=True)
    ...
    op.create_index("idx_folders_path", "folders", ["path"], if_not_exists=True)
    ...
    op.create_index(
        "idx_users_created_at", "users", [sa.text("created_at DESC")], if_not_exists=True
    )
    ...
    op.create_index("idx_audit_event_type", "audit_logs", ["event_type"], if_not_exists=True)

    # Optimize time-based queries for compliance reports
    # Used in: compliance_reports, audit_trail
    op.create_index(
        "idx_audit_timestamp", "audit_logs", [sa.text("timestamp DESC")], if_not_exists=True
    )

    # Composite index for resource-specific audit queries
    # Optimizes: SELECT * FROM audit_logs WHERE resource_type = ? AND resource_id = ?
    op.create_index(
        "idx_audit_resource", "audit_logs", ["resource_type", "resource_id"], if_not_exists=True
    )
    ...
    op.create_index(
        "idx_share_links_token", "share_links", ["share_token"], unique=True, if_not_exists=True
    )

    # Optimize file-based share link queries
    # Used in: list_file_shares, manage_shares
    op.create_index("idx_share_links_file_id", "share_links", ["file_id"], if_not_exists=True)

    # Optimize active share link queries
    # Used in: active_shares_list, cleanup_expired
    op.create_index("idx_share_links_active", "share_links", ["is_active"], if_not_exists=True)
    ...
    op.create_index("idx_activity_action", "activity_logs", ["action"], if_not_exists=True)
...
def downgrade():
    ...
    op.drop_index(
        "idx_optimization_suggestions_impact", table_name="optimization_suggestions", if_exists=True
    )
    op.drop_index(

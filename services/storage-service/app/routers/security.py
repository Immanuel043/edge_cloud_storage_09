    ...
    async with async_session() as session:
        ...
        try:
            ...
            await audit_service.log_action(
                ...
                risk_level='critical' if scan_result.risk_score > 80 else 'high' if scan_result.risk_score > 50 else 'low'
            )

            return {
                'file_id': str(file_id),
                'dlp_result': scan_result.to_dict()
            }
...
async def get_security_dashboard(
    ...
    from app.models.database import VirusScanLog, DLPScanLog, AuditLog
    ...
    async with async_session() as session:
        ...
        virus_scans_result = await session.execute(
            select(func.count(VirusScanLog.id)).where(
                VirusScanLog.user_id == current_user.id
            )
        )
        ...
        dlp_scans_result = await session.execute(
            select(func.count(DLPScanLog.id)).where(
                DLPScanLog.user_id == current_user.id
            )
        )
        ...
        return {
            ...
            'scanner_status': {
                'clamav_available': await get_virus_scanner().ping()
            }
        }

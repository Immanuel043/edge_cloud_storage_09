...
class EnhancedConnectionManager:
    ...
    def disconnect(self, websocket: WebSocket):
        ...
        if user_id:
            ...
            logger.info(
                f"WebSocket disconnected for user: {user_id} (total: {self.total_connections})"
            )
    ...
    def _blocking_listener():
        ...
        while True:
            try:
                ...
                logger.info(
                    "Subscribed to preview_notifications + file_notifications Redis channels"
                )
                backoff = 1
                ...
                for message in ps.listen():
                    ...
                    try:
                        ...
                        if channel == "file_notifications":
                            ...
                            logger.info(
                                f"Pushed preview notification (status={data.get('status')}) to user {user_id} for file {data.get('file_id')}"
                            )
        ...
        while True:
            ...
            try:
                data = await asyncio.wait_for(
                    websocket.receive_json(), timeout=60.0  # 60 second timeout
                )
            except asyncio.TimeoutError:
...
async def get_websocket_metrics(current_user=Depends(get_current_user)):  # Optional: require auth

...
async def create_folder(
    ...
    folder_path = (
        os.path.join(parent_path, folder_data.name)
        if parent_path != "/"
        else f"/{folder_data.name}"
    )
    ...
    await log_activity(
        db, current_user.id, "folder_created", str(folder.id), {"name": folder_data.name}, request
    )
...
async def update_folder(
    ...
    await log_activity(
        db, current_user.id, "folder_updated", str(folder_id), {"name": folder.name}, request
    )
...
async def delete_folder(
    ...
    if not force:
        ...
        subfolders = await db.execute(select(Folder).filter(Folder.parent_id == folder_id))
        ...
        files = await db.execute(select(Object).filter(Object.folder_id == folder_id))
        ...
        deleted_file_ids, deleted_folder_ids = await delete_folder_contents_recursive(

...
from ..models.database import Folder, Object, ShareBundle, ShareBundleFile, SharedAccess, User
from ..models.schemas import (
    ...
    ShareBundlePublicInfo,
    ShareBundleRemoveFiles,
    ShareBundleResponse,
    ShareBundleUpdate,
)
...
async def get_files_in_folder_recursive(
    db: AsyncSession, folder_id: str, user_id, base_path: str = ""
) -> List[tuple]:
    """
    ...
    files_result = await db.execute(
        select(Object).filter(
            Object.folder_id == folder_id, Object.user_id == user_id, Object.is_deleted == False
        )
    )
...
async def create_share_bundle(
    ...
    for folder_id in folder_ids:
        folder_files = await get_files_in_folder_recursive(db, folder_id, current_user.id, "")
...
async def get_share_bundle(
    ...
    files_result = await db.execute(
        ...
        .filter(ShareBundleFile.bundle_id == bundle.id, Object.is_deleted == False)
...
async def delete_share_bundle(
    ...
    await db.execute(delete(ShareBundleFile).where(ShareBundleFile.bundle_id == bundle.id))
...
async def add_files_to_bundle(
    ...
    max_order_result = await db.execute(
        select(func.max(ShareBundleFile.display_order)).filter(
    ...
    for idx, file_obj in enumerate(files):
        ...
        existing = await db.execute(
            select(ShareBundleFile).filter(
                ShareBundleFile.bundle_id == bundle.id, ShareBundleFile.file_id == file_obj.id
            )
        )
...
async def remove_files_from_bundle(
    ...
    files_result = await db.execute(
        ...
        .filter(ShareBundleFile.bundle_id == bundle.id, Object.id.in_(file_data.file_ids))
    )
    ...
    await db.execute(
        delete(ShareBundleFile).where(
            ShareBundleFile.bundle_id == bundle.id, ShareBundleFile.file_id.in_(file_data.file_ids)
        )
    )
...
@router.post(
    "/share-bundles/{bundle_id}/collaborate", response_model=List[CollaborativeShareResponse]
)
async def share_bundle_with_users(
    ...
    for email in share_data.emails:
        ...
        recipient_result = await db.execute(select(User).filter(User.email == email))
...
async def get_public_bundle_info(
    ...
    logger.info(
        f"Bundle {bundle.id}: found {len(files_data)} visible files after join (bundle claims {bundle.file_count})"
    )
...
async def stream_bundle_file(
    ...
    file_in_bundle = await db.execute(
        select(ShareBundleFile).filter(
            ShareBundleFile.bundle_id == bundle.id, ShareBundleFile.file_id == file_id
        )
    )
    ...
    if file_obj.encryption_mode == "client_zk":
        raise HTTPException(
            status_code=404,
            detail="This file uses zero-knowledge encryption and cannot be previewed from a shared bundle",
        )

    background_tasks.add_task(
        log_share_access, request, "stream", bundle_id=bundle.id, file_id=file_obj.id
    )
    ...
    # SINGLE FILE STORAGE
        ...
        if parsed_range:
            ...
            return StreamingResponse(
                generator, status_code=206, headers=headers, media_type=mime_type
            )
        else:
            ...
            return StreamingResponse(
                generator, status_code=200, headers=headers, media_type=mime_type
            )
...
async def download_bundle_file(
    ...
    if file_obj.encryption_mode == "client_zk":
        raise HTTPException(
            status_code=404,
            detail="This file uses zero-knowledge encryption and cannot be downloaded from a shared bundle",
        )
    ...
    background_tasks.add_task(
        log_share_access, request, "download_file", bundle_id=bundle.id, file_id=file_obj.id
    )
...
async def download_bundle_as_zip(
    ...
    # Get bundle
    result = await db.execute(
        select(ShareBundle).filter(ShareBundle.share_token == token, ShareBundle.is_active == True)
    )
    bundle = result.scalar_one_or_none()
    ...
    if not files_data:
        raise HTTPException(
            status_code=404, detail="No files in bundle (all files may have been deleted)"
        )
...
async def get_bundle_file_thumbnail(
    ...
    background_tasks.add_task(
        log_share_access, request, "thumbnail", bundle_id=bundle.id, file_id=file_obj.id
    )
    ...
    try:
        ...
        if cached_preview:
            return Response(
                ...
                headers={"Cache-Control": "public, max-age=86400", "X-Preview-Status": "cached"},
            )
    except Exception:
                ...
                _json.dumps(
                    {
                        ...
                        "upload_id": (
                            str((_file_obj.chunk_info or {}).get("upload_id", _file_id))
                            if _file_obj.chunk_info
                            else str(_file_id)
                        ),
    ...
    try:
        ...
        if result.status == "deferred":
            ...
            queued = await _queue_share_preview_to_kafka(file_id, file_obj, redis, status_key)
            if queued:
                return _return_share_placeholder(file_obj.mime_type)
            # Kafka unavailable — fall through to 404
            raise HTTPException(
                status_code=404, detail="Could not generate thumbnail (deferred, Kafka unavailable)"
            )
        ...
        if preview_bytes:
            ...
            try:
                from ..services.preview_storage import async_save_preview_to_disk as _async_save
                from ..services.preview_storage import (
                    preview_cache_ttl,
                )

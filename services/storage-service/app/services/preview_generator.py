...
class PreviewGenerator:
    ...
    async def _generate_video_preview(
        ...
        if HAS_FFMPEG:
            try:
                result = await self._generate_video_preview_ffmpeg(
                    file_path, size, timeout=ffmpeg_timeout
                )
        ...
        def _extract_frame_ffmpeg():
            ...
            try:
                ...
                cmd = [
                    "ffmpeg",
                    "-ss",
                    "5",  # Seek to 5 seconds (BEFORE input)
                    "-i",
                    file_path,  # Input file
                    "-frames:v",
                    "1",  # Extract exactly 1 frame
                    "-vf",
                    f"scale={size[0]}:{size[1]}:force_original_aspect_ratio=decrease",  # Scale during decode
                    "-q:v",
                    "5",  # Medium quality (faster than q:v 2)
                    "-f",
                    "image2",  # Format
                    "-y",  # Overwrite output
                    "-loglevel",
                ...
                if result.returncode != 0 or not os.path.exists(output_path):
                    ...
                    if "moov atom not found" in stderr_text.lower():
                        ...
                        raise Exception(
                            f"moov atom not found - video preview requires full metadata"
                        )
                ...
                if result.returncode != 0 or not os.path.exists(output_path):
                    ...
                    logger.info(
                        f"Second attempt failed (at 0.5s): {stderr_text_2[:200]}. Trying first frame extraction..."
                    )
                    cmd = [
                        "ffmpeg",
                        "-i",
    ...
    async def _generate_text_preview(
        self, file_path: str, size: Tuple[int, int], is_code: bool = False
    ) -> Tuple[bytes, str]:
    ...
    def _create_text_image(self, text: str, size: Tuple[int, int], title: str = "Preview") -> bytes:
    ...
    def _generate_placeholder_preview(
        self, file_type: str, size: Tuple[int, int], icon: str = "📄"
    ) -> Tuple[bytes, str]:

...
class ARQStreamingService:
    ...
    async def init_stream_session(
        ...
        """
        Initialize a new ARQ streaming session.
        ...
        logger.info(
            ...
            f"(file={file_id}, size={file_size}, chunks={total_chunks})"
        )
    ...
    async def get_chunk(
        ...
        async with aiofiles.open(session.file_path, 'rb') as f:
            ...
            data = await f.read(byte_end - byte_start + 1)
    ...
    async def request_retransmit(
        ...
        for idx in chunk_indices:
            if 0 <= idx < session.total_chunks:
                chunk = await self.get_chunk(session_id, idx)
                chunks.append(chunk)
            else:
                logger.warning(

...
from .url_validator import url_validator, SSRFProtectionError
    ...
    async def download_from_url(
        ...
        try:
            ...
            await self._update_job_status(
                redis_client, job_id, "downloading", progress=0
            )
            ...
            async with httpx.AsyncClient(
                ...
                headers={
                    'User-Agent': 'EdgeCloudStorage/1.0 (Automated Download Service)'
                }
            ) as client:
                ...
                async with client.stream('GET', url) as response:
                    ...
                    if content_length:
                        ...
                        await self._update_job_status(
                            redis_client, job_id, "downloading",
                            total_size=size
                        )
                    else:
                        size = 0
                    ...
                    logger.info(
                        f"URL download completed: {job_id}, file: {result['file_id']}"
                    )
            ...
            await self._update_job_status(
                redis_client, job_id, "failed", error=error_msg
            )

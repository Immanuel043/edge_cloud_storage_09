...
class SmartDeduplicationClassifier:
    ...
    SKIP_EXTENSIONS = {
        ...
        '.mp4', '.mkv', '.avi', '.mov', '.m4v', '.wmv', '.webm', '.flv', '.mpeg', '.mpg',
        '.3gp', '.3g2', '.f4v', '.asf', '.rm', '.rmvb', '.vob', '.ogv', '.mts', '.m2ts',

        # Media (compressed) - Audio
        '.mp3', '.m4a', '.aac', '.ogg', '.opus', '.flac', '.wma', '.ape', '.mpc',
        ...
        '.doc', '.xls', '.ppt',  # Old Office (often compressed internally)

        # Archives & compressed streams
        '.zip', '.rar', '.7z', '.gz', '.bz2', '.xz', '.zst', '.lz', '.lz4',
        '.tar.gz', '.tar.xz', '.tar.zst', '.tar.bz2', '.tgz', '.tbz2',
        '.cab', '.iso.gz', '.lzma', '.sz', '.z', '.Z',
        '.zipx', '.ace', '.arj', '.lha', '.lzh',  # Less common archives

        # Package formats (already packed & compressed)
        '.jar', '.war', '.ear', '.whl', '.gem', '.nupkg', '.deb', '.rpm',
        '.apk', '.ipa', '.app', '.pkg', '.msi', '.exe',  # Installers (often compressed)
        '.snap', '.flatpak', '.appimage',  # Linux packages

        # Encrypted/containers
        '.gpg', '.age', '.kdbx', '.hc', '.tc', '.aes', '.enc', '.encrypted',
        '.pgp', '.p12', '.pfx', '.keystore', '.jks',
        '.luks', '.dmcrypt',  # Disk encryption

        # Already deduplicated/content-addressed formats
        '.git', '.hg', '.svn',  # VCS repositories

        # Other pre-compressed
        '.epub', '.mobi', '.azw3',  # eBooks (ZIP-based)
        '.docm', '.xlsm', '.pptm',  # Office with macros (ZIP-based)

        # Compressed fonts
        '.woff', '.woff2', '.otf', '.ttc',

        # Game/binary formats (compressed)
        '.pak', '.vpk', '.bsa', '.ba2',  # Game archives
        '.unity3d', '.unitypackage',  # Unity assets
    }

    # High-value extensions - large, repetitive, good dedup candidates
    HIGH_VALUE_EXTENSIONS = {
        # VMs & disk images
        '.vmdk', '.vdi', '.vhd', '.vhdx', '.qcow2', '.iso', '.img',
        '.wim', '.esd', '.ovf', '.ova', '.raw', '.vfd',

        # Databases & logs
        '.sqlite', '.db', '.mdb', '.accdb', '.ndf', '.ldf', '.ibd', '.frm',
        '.wal', '.xlog', '.binlog', '.rocksdb', '.leveldb',
        '.dbf', '.sdf', '.fdb',  # Additional DB formats

        # Backups & archives (uncompressed)
        '.sql', '.dump', '.backup', '.bak', '.mbox', '.pst', '.ost',
        '.tar', '.cpio',  # Uncompressed tarballs
        '.img', '.toast',  # Disk images

        # Code & text/data
        '.c', '.h', '.cpp', '.hpp', '.cc', '.cxx', '.hh',
        '.go', '.rs', '.cs', '.php', '.rb', '.pl', '.sh', '.bash', '.zsh', '.fish',
        '.py', '.pyi', '.java', '.scala', '.kt', '.groovy', '.clj', '.cljs',
        '.js', '.jsx', '.ts', '.tsx', '.mjs', '.cjs', '.vue', '.svelte',
        '.json', '.yaml', '.yml', '.toml', '.xml', '.html', '.htm', '.xhtml',
        '.log', '.txt', '.md', '.markdown', '.rst', '.asciidoc', '.adoc',
        '.csv', '.tsv', '.ndjson', '.jsonl', '.map',
        '.sql', '.graphql', '.proto', '.thrift',
        '.vim', '.vimrc', '.bashrc', '.zshrc', '.profile',
        ...
        # Scientific data
        '.h5', '.hdf5', '.parquet', '.feather', '.arrow', '.nc', '.cdf',
        '.npz', '.npy', '.mat', '.fits', '.sav', '.rda', '.rds',
        '.pickle', '.pkl',  # Python serialized data

        # Uncompressed media
        '.wav', '.aiff', '.aif', '.au', '.snd', '.pcm',  # Audio
        '.bmp', '.tif', '.tiff', '.pcx', '.ppm', '.pgm', '.pbm', '.pnm',  # Images
        '.yuv', '.raw', '.dng',  # Raw video/photo

        # RAW photos (often large with repeating patterns)
        '.cr2', '.cr3', '.nef', '.arw', '.orf', '.rw2', '.dng',
        '.raf', '.pef', '.srw', '.x3f', '.mrw', '.dcr', '.kdc', '.erf',

        # CAD/Design (often huge and repetitive)
        '.psd', '.psb', '.ai', '.indd', '.sketch', '.xd', '.fig',
        '.dwg', '.dxf', '.dwf', '.dgn', '.ifc',
        '.rvt', '.rfa', '.rte', '.rft',
        '.step', '.stp', '.iges', '.igs', '.stl', '.3dm', '.3ds',
        '.blend', '.max', '.fbx', '.obj', '.dae', '.gltf', '.glb',
        '.ma', '.mb',  # Maya files

        # Documentation (often duplicated)
        '.tex', '.bib', '.docm', '.dotx', '.rtf', '.odt',
        ...
        '.shp', '.shx', '.dbf', '.prj', '.gdb', '.kml', '.kmz', '.geojson',

        # Genomics (large text files)
        '.fasta', '.fastq', '.fq', '.fa', '.vcf', '.bam', '.sam', '.bed', '.gff', '.gtf',
    ...
    async def _detect_compression_encryption(
        self,
        file_path: str
    ) -> dict:
        """
    ...
    async def _classify_tar(
        ...
        return FileClassification(
            dedup_mode='inline',
            reason='medium_tar_uncompressed',
            priority=1,
            chunk_strategy='cdc'
        )

    def _classify_high_value(
        self,
        file_size: int,
        ext: str
    ) -> FileClassification:

...
from ..models.database import ContentBlock, Object
...
class DeduplicationService:
    ...
    async def deduplicate_file(self, file_data: bytes, file_name: str, db: AsyncSession) -> Dict:
        """
        ...
        for i in range(0, total_size, self.block_size):
            ...
            blocks.append(
                {"hash": block_hash, "size": block_size, "offset": i, "is_duplicate": is_duplicate}
            )
        ...
        existing_file = await db.execute(select(Object).where(Object.content_hash == file_hash))
    ...
    async def retrieve_file_from_cas(self, file_id: str, db: AsyncSession) -> bytes:
        """
        ...
        if user_id:
            ...
            result = await db.execute(
                select(
                    ...
                    func.sum(func.cast(Object.dedup_info["saved_size"], Integer)).label(
                        "saved_size"
                    ),
                ).where(
            ...
            result = await db.execute(
                ...
                ).where(Object.storage_type.in_(["deduplicated", "content_addressed"]))
            )
    ...
    async def cleanup_unreferenced_blocks(self, db: AsyncSession) -> int:
        ...
        if unreferenced:
            await db.execute(delete(ContentBlock).where(ContentBlock.block_hash.in_(unreferenced)))
            await db.commit()

...
class ColdStorageTieringService:
    ...
    def __init__(self):
        ...
        logger.info(f"Tiering thresholds: cache→warm: {self.cache_to_warm_days}d, "
    ...
    async def _run_tiering_cycle(self):
        ...
        async with async_session() as db:
            ...
            moved_to_warm = await self._tier_files(
                ...
                from_tier='cache',
                to_tier='warm',
                age_days=self.cache_to_warm_days
            )
            ...
            moved_to_cold = await self._tier_files(
                ...
                from_tier='warm',
                to_tier='cold',
                age_days=self.warm_to_cold_days
            )
        ...
        logger.info(f"Tiering cycle complete: {moved_to_warm} to warm, "
    ...
    async def _check_tier_capacity(self, db: AsyncSession):
        ...
        for tier_name, tier_config in self.tiers.items():
            ...
            query = select(func.sum(Object.file_size)).where(
            ...
            logger.info(f"Tier {tier_name}: {current_size_gb:.2f}GB / {max_size_gb}GB ({usage_percent:.1f}%)")
            ...
            if usage_percent > 90 and tier_config['next_tier']:
                ...
                query = select(Object).where(
                    Object.storage_tier == tier_name
                ).order_by(Object.last_accessed.asc()).limit(50)

                result = await db.execute(query)
                oldest_files = result.scalars().all()

                for file_obj in oldest_files:
                    await self._move_file_to_tier(
                        file_obj,
                        tier_config['next_tier'],
                        db
                    )
    ...
    async def promote_file_on_access(self, file_id: str, db: AsyncSession):
        ...
        try:
            ...
            if file_obj.storage_tier in ['warm', 'cold']:
                logger.info(f"Promoting file {file_obj.file_name} from "

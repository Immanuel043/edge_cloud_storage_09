...
class SearchService:
    ...
    async def index_file(self, file_data: Dict[str, Any], retries: int = 3) -> bool:
        ...
        if not self.connected or self.client is None:
            logger.warning(
                f"Elasticsearch not connected, skipping file indexing for: {file_data.get('name', 'unknown')}"
            )
            return False
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    ...
                    logger.warning(
                        f"Failed to index file (attempt {attempt + 1}/{retries}): {e}. Retrying in {wait_time}s..."
                    )
    ...
    async def index_folder(self, folder_data: Dict[str, Any], retries: int = 3) -> bool:
        ...
        if not self.connected or self.client is None:
            logger.warning(
                f"Elasticsearch not connected, skipping folder indexing for: {folder_data.get('name', 'unknown')}"
            )
            return False
        ...
        for attempt in range(retries):
            try:
                await self.client.index(
                    index=self.folders_index, id=str(folder_data["id"]), document=doc
                )
                ...
                if attempt < retries - 1:
                    ...
                    logger.warning(
                        f"Failed to index folder (attempt {attempt + 1}/{retries}): {e}. Retrying in {wait_time}s..."
                    )
                    await asyncio.sleep(wait_time)
                else:
    ...
    async def search(
        ...
        try:
            ...
            if query:
                ...
                should_clauses.append({"term": {"name.keyword": {"value": query, "boost": 100.0}}})

                # 2. Case-insensitive exact match (50x boost)
                should_clauses.append(
                    {"term": {"name.keyword": {"value": query.lower(), "boost": 50.0}}}
                )

                # 3. Prefix match (starts with query - 20x boost)
                should_clauses.append({"prefix": {"name": {"value": query.lower(), "boost": 20.0}}})

                # 4. Phrase match - all words in order (10x boost)
                # This matches "renewalpdf (Copy)" as a phrase, not individual words
                should_clauses.append({"match_phrase": {"name": {"query": query, "boost": 10.0}}})
                ...
                should_clauses.append(
                    {"wildcard": {"name": {"value": f"*{safe_query}*", "boost": 5.0}}}
                )
    ...
    async def update_file_text(self, file_id: str, ocr_text: str) -> bool:
        ...
        if not self.connected or self.client is None:
            logger.warning(
                f"Elasticsearch not connected, skipping OCR text update for file: {file_id}"
            )
            return False
        ...
        try:
            await self.client.update(
                index=self.files_index,
                id=str(file_id),
                body={"doc": {"ocr_text": ocr_text, "searchable_content": ocr_text}},
            )
    ...
    async def update_file_tags(self, file_id: str, tags: list[str]) -> bool:
        ...
        try:
            await self.client.update(
                index=self.files_index, id=str(file_id), body={"doc": {"tags": tags}}
            )
    ...
    async def delete_file(self, file_id: str) -> bool:
        ...
        if not self.connected or self.client is None:
            logger.warning(
                f"Elasticsearch not connected, skipping file deletion from index: {file_id}"
            )
            return False
    ...
    async def delete_folder(self, folder_id: str) -> bool:
        ...
        if not self.connected or self.client is None:
            logger.warning(
                f"Elasticsearch not connected, skipping folder deletion from index: {folder_id}"
            )
            return False

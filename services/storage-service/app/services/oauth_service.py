...
class OAuthService:
    ...
    @staticmethod
    async def create_or_update_user(db: AsyncSession, oauth_profile: Dict, provider: str):
        """
        ...
        result = await db.execute(
            select(OAuthAccount).filter(
                OAuthAccount.provider == provider, OAuthAccount.provider_user_id == oauth_sub
            )
        )
    ...
    async def link_oauth_to_existing_user(
        db: AsyncSession, user_id: str, oauth_profile: Dict, provider: str
    ):
        """

...
import aiohttp
import base64
import hashlib as _hashlib  # to derive master key
...
class Settings:
    ...
    if not SECRET_KEY:
        # For local dev you can fall back, but in prod keep this required.
    ...
    # Chunking - Read from environment variables
    CHUNK_SIZE = int(os.getenv("CHUNK_SIZE", 67108864))  # Default 64MB
    MAX_FILE_SIZE = int(os.getenv("MAX_FILE_SIZE", 21474836480))  # Default 20GB
...
class User(Base):
    ...
    user_type = Column(String(20), default="individual")  # individual, business, enterprise
...
class Object(Base):
    ...
    storage_type = Column(String(20), default="chunked")  # inline, single, chunked
    storage_key = Column(String(500))   # For inline storage
    object_path = Column(String(500))   # For single object storage
...
class EncryptionService:
    """
    - Generates a random per-file Fernet key (data key)
    - Wraps/unwraps that key with a master Fernet derived from SECRET_KEY
    """

    def __init__(self):
        # Derive a stable 32-byte key from SECRET_KEY
        master = _hashlib.sha256(settings.SECRET_KEY.encode()).digest()
        fernet_key = base64.urlsafe_b64encode(master)  # 32 bytes -> Fernet key format
        self._keywrap = Fernet(fernet_key)
    ...
    def encrypt_key(self, file_key: bytes) -> str:
        return self._keywrap.encrypt(file_key).decode()

    def decrypt_key(self, encrypted_key: str) -> bytes:
        return self._keywrap.decrypt(encrypted_key.encode())
...
class StorageService:
    # Storage thresholds
    INLINE_THRESHOLD = 1 * 1024 * 1024           # 1MB - store in Redis
    SINGLE_OBJECT_THRESHOLD = 100 * 1024 * 1024  # 100MB - store as single file
    ...
    async def store_file_auto(self, file_data: bytes, file_info: dict, encrypt_key: bytes) -> dict:
        ...
        elif file_size < self.SINGLE_OBJECT_THRESHOLD:
            return await self.store_single(file_data, file_info, encrypt_key)
        else:
            # Use chunking for large files (existing method)
    ...
    async def store_inline(self, file_data: bytes, file_info: dict, encrypt_key: bytes) -> dict:
        """Store tiny files directly in Redis"""
    ...
    async def store_single(self, file_data: bytes, file_info: dict, encrypt_key: bytes) -> dict:
        """Store medium files as single objects"""
        file_hash = hashlib.sha256(file_data).hexdigest()
    ...
    async def save_chunk(
        ...
        # Compress (app-level; consider disabling if FS handles compression)
        ...
        # Store metadata WITHOUT TTL (persistent until refs=0)
    ...
    async def move_to_tier(self, chunk_hash: str, target_tier: str):
        ...
        # Move file (copy+delete across filesystems is expected)
...
class BackupService:
    ...
    async def backup_object(self, object_id: str, chunks: List[Dict]):
        ...
        NOTE: backs up the stored representation (compressed/encrypted chunk bytes)
        by concatenating raw chunk files. For true logical restore, you'd restore
        chunk files individually or stream through decrypt+decompress.
        """
        backup_path = os.path.join(settings.BACKUP_PATH, str(object_id))
        os.makedirs(backup_path, exist_ok=True)

        # Collect all chunks (RAW on-disk bytes)
        full_data = b""
...
async def register(
    ...
    await log_activity(
        db, user.id, "user_registered", metadata={"user_type": user_type}, request=request
    )
...
async def get_file_preview(
    ...
    if not file_obj.mime_type or not file_obj.mime_type.startswith('image/'):
        raise HTTPException(400, "Preview only available for images")
    ...
    async def stream_preview():
        if file_obj.storage_type == "inline":
            data = await redis_client.get(file_obj.storage_key)
            if file_key:
                data = encryption_service.decrypt_data(data, file_key)
...
async def create_folder(
    ...
    if parent_id:
        result = await db.execute(
            select(Folder).filter(Folder.id == parent_id, Folder.user_id == current_user.id)
        )
    ...
    folder_path = os.path.join(parent_path, name) if parent_path != "/" else f"/{name}"
    
    folder = Folder(
        user_id=current_user.id,
        parent_id=parent_id,
        name=name,
        path=folder_path,  # <-- Use the fixed path
    )
    db.add(folder)
    await db.commit()

    await log_activity(db, current_user.id, "folder_created", str(folder.id), {"name": name}, request)
...
async def download_file(
    ...
    """Download file with support for all storage types"""
    result = await db.execute(
        select(Object).filter(Object.id == file_id, Object.user_id == current_user.id)
    )
    file_obj = result.scalar_one_or_none()
    ...
    async def stream_file():
        if file_obj.storage_type == "inline":
            ...
            data = await redis_client.get(file_obj.storage_key)
            if not data:
                raise HTTPException(404, "File data not found in cache")
    ...
    await log_activity(db, current_user.id, "file_downloaded", str(file_id), request=request)
...
async def create_share_link(
    ...
    await redis_client.setex(f"share:{share_token}", expires_hours * 3600, json.dumps(share_data))
    ...
    "expires_hours": expires_hours,  # Add this
    "password": bool(password),  # Change to just boolean
    "max_downloads": max_downloads,  # Add this
}
...
@app.post("/api/v1/files/bulk-delete")
async def bulk_delete_files(
    ...
    """Delete multiple files with support for all storage types"""
    deleted_count = 0
    freed_space = 0
    ...
    return [
        {
            ...
            "metadata": a.meta_data,  # fixed field name
...
async def init_upload(
    ...
    # Decide storage strategy upfront
    if file_size < storage_service.INLINE_THRESHOLD:
        storage_strategy = "inline"
        total_chunks = 0  # No chunking needed
    elif file_size < storage_service.SINGLE_OBJECT_THRESHOLD:
    ...
    session_data = {
        ...
        "file_size": file_size,
        "folder_id": folder_id,
        "storage_strategy": storage_strategy,  # New field
    ...
    return {
        ...
        "chunk_size": settings.CHUNK_SIZE if storage_strategy == "chunked" else 0,
        "total_chunks": total_chunks,
        "direct_upload": storage_strategy != "chunked"  # Frontend can upload whole file
    }
...
async def upload_chunk(
    ...
    if len(session["chunk_hashes"]) <= chunk_index:
        session["chunk_hashes"].extend([None] * (chunk_index + 1 - len(session["chunk_hashes"])))
...
async def complete_upload(
    ...
    """Complete upload with support for all storage strategies"""
    ...
    file_id = uuid.uuid4()
    mime_type = mimetypes.guess_type(session["file_name"])[0]
    
    # Different handling based on storage strategy
    storage_strategy = session.get("storage_strategy", "chunked")
    
    if storage_strategy == "inline":
        # For inline storage, the file data should be in session
        file_data = session.get("file_data")
        if not file_data:
            raise HTTPException(400, "File data missing for inline storage")
        
        file_key = encryption_service.decrypt_key(session["encryption_key"])
        storage_result = await storage_service.store_inline(
        ...
        file_obj = Object(
            ...
            content_hash=storage_result["hash"],
            encryption_key=session["encryption_key"],
            storage_tier="cache",
        )
        
    elif storage_strategy == "single":
        # For single object storage
        file_data = session.get("file_data")
        if not file_data:
            raise HTTPException(400, "File data missing for single object storage")
        
        file_key = encryption_service.decrypt_key(session["encryption_key"])
        storage_result = await storage_service.store_single(
            base64.b64decode(file_data),
            {"user_id": str(current_user.id)},
            file_key
        )
        ...
        file_obj = Object(
            ...
            mime_type=mime_type,
            storage_type="single",
            object_path=storage_result["path"],
            content_hash=storage_result["hash"],
            encryption_key=session["encryption_key"],
            storage_tier="cache",
        )
        ...
        file_obj = Object(
            ...
            encryption_key=session["encryption_key"],
            chunk_info={"chunks": session["chunk_hashes"], "count": session["total_chunks"]},
            storage_tier="cache",
        )
    ...
    await log_activity(
        ...
        {"file_name": session["file_name"], "size": session["file_size"], "storage_type": storage_strategy},
        request,
    )

    # Optional: trigger backup asynchronously
    #if settings.BACKUP_ENABLED and background_tasks:
     #   chunk_meta_list = [{"hash": h} for h in session["chunk_hashes"]]
      #  background_tasks.add_task(backup_service.backup_object, str(file_id), chunk_meta_list)
...
@app.post("/api/v1/upload/direct/{upload_id}")
async def upload_direct(
    ...
    """Direct upload for small and medium files"""
    ...
    # Store in session for complete_upload to process
    session["file_data"] = base64.b64encode(file_data).decode()
...
async def get_upload_status(
    ...
    missing_chunks = set(range(session["total_chunks"])) - set(session["uploaded_chunks"])
...
if __name__ == "__main__":
    import uvicorn
    # In production use: `uvicorn services.storage-service.app.main:app --host 0.0.0.0 --port 8000`

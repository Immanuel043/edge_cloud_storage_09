...
class FileVersion(Base):
    ...
    __table_args__ = (UniqueConstraint("file_id", "version_number", name="unique_file_version"),)
...
class ContentBlock(Base):
    ...
    __table_args__ = (Index("idx_cb_block_hash", "block_hash"),)
...
class FileBlockMapping(Base):
    ...
    block_id = Column(
        UUID(as_uuid=True), ForeignKey("content_blocks.id"), nullable=False, index=True
    )
...
class ShareLink(Base):
    ...
    file_id = Column(
        UUID(as_uuid=True), ForeignKey("objects.id", ondelete="CASCADE"), nullable=True
    )
    folder_id = Column(
...
class ShareBundleFile(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    bundle_id = Column(
        UUID(as_uuid=True), ForeignKey("share_bundles.id", ondelete="CASCADE"), nullable=False
    )
    file_id = Column(
...
class SharedAccess(Base):
    ...
    bundle_id = Column(
        UUID(as_uuid=True), ForeignKey("share_bundles.id", ondelete="CASCADE"), nullable=True
    )
...
class ShareAccessLog(Base):
    ...
    bundle_id = Column(
        UUID(as_uuid=True), ForeignKey("share_bundles.id", ondelete="SET NULL"), nullable=True
    )
    access_type = Column(
        String(30), nullable=False
    )  # view, download, download_file, download_zip, stream, thumbnail
    ip_address = Column(String(45), nullable=True)  # IPv4/IPv6
    user_agent = Column(Text, nullable=True)
    file_id = Column(
        UUID(as_uuid=True), nullable=True
    )  # Specific file accessed (for per-file events)
...
class FileOCR(Base):
    ...
    __table_args__ = (Index("idx_file_ocr_file_id", "file_id"),)
...
class QuotaAlert(Base):
    ...
    predicted_days_remaining = Column(
        Integer
    )  # Days until predicted full (for predicted_full alerts)
...
class OptimizationSuggestion(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    analysis_id = Column(
        UUID(as_uuid=True), ForeignKey("storage_analysis.id", ondelete="CASCADE"), nullable=False
    )
...
class OrganizationCluster(Base):
    ...
    )  # Generated name (e.g., "Photos 2024", "Work Documents")
...
class FileClusterAssignment(Base):
    ...
    cluster_id = Column(
        UUID(as_uuid=True),
        ForeignKey("organization_clusters.id", ondelete="CASCADE"),
        nullable=False,
    )
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
...
class FileEmbedding(Base):
    ...
    source_type = Column(
        String(50), default="filename"
    )  # 'filename', 'content', 'tags', 'combined'
...
class UserInteraction(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "user_id", "file_id", "interaction_type", name="unique_user_file_interaction"
        ),
    )
...
class Recommendation(Base):
    ...
    recommendation_type = Column(
        String(50), nullable=False
    )  # 'similar', 'collaborative', 'trending', 'personalized'
...
class EncryptionKeyVersion(Base):
    ...
    __table_args__ = (Index("idx_key_version_status", "version", "status"),)
